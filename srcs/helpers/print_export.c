...
/*   Updated: 2020/10/23 18:21:11 by mklotz           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/minishell.h"

static void	get_emty(int **already_exist, int size, int min_val_index)
{
	int i;
...
static char	*get_next_str(int **already_exist, char **env, int size)
{
	...
	while (i < size)
	{
		if (!is_already_exist(already_exist, i, size))
		{
			...
			{
				if (ft_strncmp(env[min_val_index], env[i],
				ft_strlen(env[min_val_index])) > 0)
					min_val_index = i;
			}
		}
		i++;
	}
	get_emty(already_exist, size, min_val_index);
